...
from utils.env import project_domain,project_name,os_home
import subprocess
...
def sync_chaincode_package(chaincode_name, host_package, host_sync_list):
    logger.info(f"sync {chaincode_name} package from {host_package} to {' '.join(host_sync_list)}...")
    ...
    with Connection(host_package) as c:
        c.get(f"{os_home}/{project_name}/{chaincode_name}.tar.gz", f"{os_home}/{project_name}/{chaincode_name}.tar.gz")
    for host in host_sync_list:
        with Connection(host) as c:
            c.put(f"{os_home}/{project_name}/{chaincode_name}.tar.gz", f"{os_home}/{project_name}/{chaincode_name}.tar.gz")
...
def install_chaincode(channel_name, chaincode_name, host_install_list, org_install_list):
...
def query_chaincode_commitment(channel_name, chaincode_name, host_query_list, org_query_list):
    for i in range(len(host_query_list)):
        ...
        logger.info(f"query chaincode {chaincode_name} commitment for {org} on {host}...")
        logger.info(f"command is :\n {command}")
        remote_run(host, command)
...
def query_chaincode_all(channel_name, chaincode_name, host_query_list, org_query_list, query_args):
    for i in range(len(host_query_list)):
        host = host_query_list[i]
        org = org_query_list[i]
        query_chaincode(channel_name, chaincode_name, host, org,query_args)  

def deploy_chaincode(channel_name, chaincode_name, chaincode_path, host_package, hosts, orgs, query_args):
...
def invoke_chaincode(channel_name, chaincode_name, host_invoke, org_invoke, invoke_args):
    ...
    logger.info(f"invoke chaincode {chaincode_name} for {org_invoke} on {host_invoke}...")
    logger.info(f"command is :\n {command}")
